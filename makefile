...
LINK_LIBS += -L./$(PNG)/lib/ -l$(PNG)
LINK_LIBS += -L./$(JPEG)/lib/ -l$(JPEG)

OBJECT_DIR = object_files
SOURCE_DIR = src
INCLUDE += -I./$(GLUI)/include 
INCLUDE += -I./$(PNG)/include#necessary?
...
$(PNG_LIB):
	cd $(PNG_PATH); ./configure --prefix=$(PWD) --enable-shared=no
	$(MAKE) -C $(PNG_PATH) all

$(JPEG_LIB):
	cd $(JPEG_PATH); ./configure --prefix=$(PWD) --enable-shared=no
